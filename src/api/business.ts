...
export type BusinessStatus = 'PENDING' | 'APPROVED' | 'REJECTED' | 'DRAFT' | 'WITHDRAWN' | 'ARCHIVED';
...
export interface BusinessReview { id: string; businessId: string; userId: string; reviewerName: string; reviewerAvatarUrl?: string | null; rating: number; comment: string; createdAt: string; }
...
export function useArchiveBusinessMutation() { const qc = useQueryClient(); return useMutation({ mutationFn: async (id: string) => normalizeBusiness(unwrap<Business>(await apiClient.patch(`/businesses/${id}/archive`))), onSuccess: (business) => { invalidate(qc); qc.invalidateQueries({ queryKey: ['business', business.id] }); } }); }
export function useUnarchiveBusinessMutation() { const qc = useQueryClient(); return useMutation({ mutationFn: async (id: string) => normalizeBusiness(unwrap<Business>(await apiClient.patch(`/businesses/${id}/unarchive`))), onSuccess: (business) => { invalidate(qc); qc.invalidateQueries({ queryKey: ['business', business.id] }); } }); }
